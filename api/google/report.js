...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const row of campaignRows) {
      ...
      const campaignClicks = Number(row.metrics.clicks || 0);
      const campaignImpressions = Number(row.metrics.impressions || 0);
      ...
      current.clicks += campaignClicks;
      current.impressions += campaignImpressions;
      current.leads += leads;
    }
    ...
    const campaigns = Array.from(campaignMap.values())
      .map((campaign) => ({
        ...
        cpc:
          campaign.clicks > 0
            ? campaign.investimento / campaign.clicks
            : 0,
        cpa:
          campaign.leads > 0
            ? campaign.investimento / campaign.leads
            : 0,
        conversion_rate:
          campaign.clicks > 0
            ? (campaign.leads / campaign.clicks) * 100
            : 0,
      }))
    ...
    for (const row of keywordRows) {
      ...
      const keywordClicks = Number(row.metrics.clicks || 0);
      const keywordImpressions = Number(row.metrics.impressions || 0);
      const keywordSpend = Number(row.metrics.cost_micros || 0) / 1_000_000;
      const keywordConversions = Number(row.metrics.conversions || 0);
      ...
      current.clicks += keywordClicks;
      current.impressions += keywordImpressions;
      current.spend += keywordSpend;
      current.conversions += keywordConversions;
    }
    ...
    for (const row of deviceRows) {
      ...
      const deviceClicks = Number(row.metrics.clicks || 0);
      const deviceImpressions = Number(row.metrics.impressions || 0);
      const deviceConversions = Number(row.metrics.conversions || 0);
      ...
      current.cost += cost;
      current.clicks += deviceClicks;
      current.impressions += deviceImpressions;
      current.conversions += deviceConversions;
    }
    ...
    return res.status(200).json({
      ...
      insights: [],
      source: "google_ads",
      debugVersion: "device-breakdown-v2",
    });
  } catch (err) {
    console.error(err);
